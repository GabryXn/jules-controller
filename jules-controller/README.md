...
- ✅ **Schedule granulare**: Ogni automazione può avere una frequenza propria (`daily`, `weekly`, `monthly`).
...
2. Esegue `scripts/sync-schedules.py`, che legge gli orari in formato **Rome Time** e li converte in **UTC** gestendo automaticamente l'offset CET (UTC+1) e CEST (UTC+2).
3. Aggiorna le righe `cron` nei file `master-setup.yml`, `controller.yml` e `auto-config-sync.yml` tramite regex.
4. Committa e pusha automaticamente i cambiamenti.

**Esempio:** Cambi `setup_sync_time` da `"03:00"` a `"05:00"` → Il cron in `master-setup.yml` diventa automaticamente `0 4 * * *` (UTC, in ora solare CET).
...
Per i repository che utilizzano Google Apps Script con TypeScript ed esbuild, Jules Controller automatizza il build e il push su Apps Script ogni volta che viene fatto un merge sul branch principale.

#### Come Funziona

1. Il Setup Universale rileva la presenza di un file `.clasp.json` nella root del repository target (o tramite `clasp_targets.yml`).
...
- 🧹 **Dead Code & Import Cleanup**: Rimozione import inutilizzati, variabili mai usate e codice commentato obsoleto.
